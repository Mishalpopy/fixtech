...
onMounted(() => {
    // console.log(props.role.permissions, "%%%%%%%");
    // console.log("########### __ ########################");
    ...
    Object.keys(props.permissions['permission']).forEach((key) => {
        ...
        props.permissions['permission'][key].forEach(element => {
            ...
            element.selected = props.role.permissions.find(el=>el.name==element.name) ? true : false;
            if(!element.selected) s =false;
        });

        props.permissions['permission'][key].selected = s;
        if(!props.permissions['permission'][key].selected) all_selected =false;
    });

    form.select_all = all_selected;
})
    ...
    <AppLayout>
        ...
        <div class="card mt-2">
            ...
            <div class="flex justify-end mt-6 mr-3">
                <Button size="large" label="Save" icon="pi pi-check" :loading="loading" @click="update()" />
            </div>
