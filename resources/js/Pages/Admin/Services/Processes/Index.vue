...
function deleteProcess(data) {
    deleteModal.value.showModal(route('admin:processes.destroy', [data.id]));
}

function createProcess() {
    const params = props.subServiceId ? { sub_service_id: props.subServiceId } : {};
    router.get(route('admin:processes.create'), params);
}

function bulkCreateProcess() {
    const params = props.subServiceId ? { sub_service_id: props.subServiceId } : {};
    router.get(route('admin:processes.bulk.create'), params);
}

function editProcess(data) {
    router.get(route('admin:processes.edit', [data.id]));
}

function showProcessDetails(data) {
    router.get(route('admin:processes.show', [data.id]));
}

function updateStatus(data) {
    router.post(
        route("admin:processes.change_status", { process: data.id }),
        {},
        {
            onSuccess: (res) => {},
            onError: (err) => {},
...
function filterBySubService() {
    if (subServiceId.value) {
        router.get(route('admin:processes.index', { sub_service_id: subServiceId.value }));
    } else {
        router.get(route('admin:processes.index'));
    }
}
...
<template>
    ...
    <AppLayout>
        ...
        <div class="card mt-2">
            ...
            <DataTable ref="dt" :value="props.processes" dataKey="id" :paginator="true" :rows="10" :filters="filters"
                    ...
                    <div class="flex flex-wrap gap-2 items-center justify-between">
                        ...
                        <div>
                            <Button label="Bulk Create" icon="pi pi-plus-circle" severity="success" class="mr-2"
                                @click="bulkCreateProcess()" />
                            <Button label="New Process" icon="pi pi-plus" severity="secondary" class="mr-2"
                                @click="createProcess()" />
                        </div>
                    </div>
                </template>
                ...
                <Column :exportable="false" field="actions" header="Actions">
                    <template #body="{ data }">
                        <Button icon="pi pi-eye" variant="text" severity="info" size="medium"
                            @click="showProcessDetails(data)" />
                        <Button icon="pi pi-pencil" variant="text" class="mr-2" size="medium" @click="editProcess(data)" />
                        <Button icon="pi pi-trash" variant="text" severity="danger" size="medium" @click="deleteProcess(data)" />
                    </template>
                </Column>
            </DataTable>
        </div>
