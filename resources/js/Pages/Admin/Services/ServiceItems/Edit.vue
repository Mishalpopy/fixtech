...
const imagePreview = ref(props.serviceItem.image ? `/storage/${props.serviceItem.image}` : null);
...
const form = useForm({
    sub_service_id: props.serviceItem.sub_service_id,
    name: props.serviceItem.name,
    description: props.serviceItem.description || '',
    price: props.serviceItem.price || null,
    image: null,
...
function removeImage() {
    form.image = null;
    imagePreview.value = props.serviceItem.image ? `/storage/${props.serviceItem.image}` : null;
    if (imageInput.value) {
...
function update() {
    loading.value = true;
    form.post(route('admin:service-items.update', [props.serviceItem.id]), {
        preserveScroll: true,
        forceFormData: true,
        _method: 'PUT',
        onSuccess: () => {
...
<template>
    <Head title="Edit Service Item" />
    <AppLayout>
        <template #title>
            <span>Edit Service Item</span>
        </template>
        <template #breadcrumb>
