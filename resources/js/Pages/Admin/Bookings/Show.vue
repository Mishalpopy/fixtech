...
import { Button, Column, DataTable, DatePicker, Divider, InputText, Message, Select, Tag, Textarea } from 'primevue';
...
<template>
    ...
    <AppLayout>
        ...
        <template #title>
            <Message>Booking #{{ props.booking.booking_number }}</Message>
        </template>


        <div class="card mt-4 mx-6">
            <Tag icon="pi pi-check" severity="success" :value="props.booking.status"
                v-if="props.booking.status == 'confirmed'">
            </Tag>
            <Tag icon="pi pi-info-circle" severity="danger" :value="props.booking.status"
            v-if="props.booking.status == 'cancelled'"></Tag>
            <Tag icon="pi pi-exclamation-triangle" severity="warn":value="props.booking.status"
            v-if="props.booking.status == 'pending'"></Tag>
